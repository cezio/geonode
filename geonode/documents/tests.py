...
if has_notifications:
    class DocumentNotificationsTestCase(NotificationsTestsHelper, TestCase):
        ...
        def setUp(self):
            ...
            self.u.save()
            notifications = self.setup_notifications_for(DocumentsAppConfig.NOTIFICATIONS, self.u)
        ...
        def testDocumentNotifications(self):
            with self.settings(NOTIFICATION_QUEUE_ALL=False):
                ...
                comment = Comment(author=self.u, name=self.u.username, content_type=lct, object_id=l.id, content_object=l, comment='test comment')
                comment.save()
